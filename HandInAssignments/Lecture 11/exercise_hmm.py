...
def compute_forward(states, observations, transitions, emissions):
    ...
    for s in inclusive_range(1, big_n):
        prob += forward[s, big_t] * transitions[s, qf]
    ...
    for t in inclusive_range(2, big_t):
        for s in inclusive_range(1, big_n):
            ...
            for s_prime in inclusive_range(1, big_n):
                viterbis_list.append(viterbi[s_prime, t-1] * transitions[s_prime, s] * emissions[s, observations[t]])
                backpointers_list.append(viterbi[s_prime, t-1] * transitions[s_prime, s])
            viterbi[s, t] = max(viterbis_list)
            backpointers[s, t] = argmax(backpointers_list) + 1


    qf_viterbi = []
    qf_backpointer = []
    for s in inclusive_range(1, big_n):
        qf_viterbi.append(viterbi[s, big_t] * transitions[s, qf])
        qf_backpointer.append(viterbi[s, big_t] * transitions[s, qf])
